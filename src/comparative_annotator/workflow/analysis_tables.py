...
def _build_edge_index(edge_rows: list[dict]) -> dict[tuple[str, str, str, str], dict]:
...
def _build_round_overview_rows(round_dir: str | Path) -> list[dict[str, Any]]:
    ...
    for row in summary_rows:
        ...
        if edge_path.exists():
            ...
            edge_rows = x.get("edges", [])
            n_edges_total = x.get("n_edges", len(edge_rows))
            n_edges_accepted = sum(1 for e in edge_rows if e.get("accepted"))
            class_counts = Counter(e.get("edge_class") for e in edge_rows if e.get("edge_class") is not None)

        rows.append(
            {
                "round_id": _safe_int(row.get("round_id")),
                "seed_species": row.get("seed_species"),
                "reference_species": reference_species,
                "target_species": target_species,
                "n_processed": _safe_int(row.get("n_processed")),
                "n_ok": _safe_int(row.get("n_ok")),
                "n_error": _safe_int(row.get("n_error")),
                "n_primary": _safe_int(row.get("n_primary")),
                "n_alternative_only": _safe_int(row.get("n_alternative_only")),
                "n_missing": _safe_int(row.get("n_missing")),
                "n_strand_conflict": _safe_int(row.get("n_strand_conflict")),
                ...
                "next_reference_species": row.get("next_reference_species"),
                "stop": _safe_boolish(row.get("stop")),
            }
        )
